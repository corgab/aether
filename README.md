...
AETHER_SUBMIT_TIMEOUT=0       # seconds one attempt of the submission job may run; 0 derives it from AETHER_PROCESS_TIMEOUT and retry_after
```

**Worker timeout**: `SubmitQuantumCircuit` declares its own `$timeout`, so the worker's default of 60 seconds (`queue:work --timeout`) does not apply to it. That matters for the local driver, which runs the whole simulation inside the submission job: a legitimate simulation of a few minutes would otherwise be killed at the one-minute mark and retried against the same limit. By default the job may run for `AETHER_PROCESS_TIMEOUT` plus a 30-second margin, so the Python bridge's own timeout error wins over the worker's; a `process_timeout` of `0` (unlimited) keeps the job unlimited too. Because the database and Redis drivers hand a job that outlives the connection's `retry_after` to a second worker, the derived value is also kept 10 seconds under the default connection's `retry_after` (90 seconds in a fresh Laravel app), which caps a local simulation at about 80 seconds until you raise `retry_after` or set `AETHER_SUBMIT_TIMEOUT` explicitly, which always wins. A timed-out attempt is failed rather than retried, whether the worker killed it (`$failOnTimeout = true`) or the Python bridge did (`PythonProcessTimedOutException`): a retry would run the same circuit against the same limit and, on AWS, could duplicate a task the backend had already accepted.
