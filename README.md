...
`Quantum::fake()` dispatches `CircuitExecuted` and `EntropyGenerated` too, and `CircuitFailed` the first time a task stubbed with `respondWithTaskStatus(TaskStatus::Failed)` or `TaskStatus::Cancelled` is polled, mirroring the real drivers, so `Event::fake()` assertions on application code keep working the same way whether or not the backend itself is faked. `CircuitExecuted` fires only for synchronous execution (`->run()` and `Quantum::batch()->run()`): a local `->dispatch()` runs the simulator inline but announces itself through `CircuitCompleted` alone, like the `aws` driver.
