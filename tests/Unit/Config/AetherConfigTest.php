...
it('casts the numeric strings env() hands over', function () {
    $config = aetherConfig(['poll_interval' => '3', 'max_poll_attempts' => '12', 'max_poll_exceptions' => '8']);
...
it('falls back to the default for a blank or non-numeric integer option', function (mixed $raw) {
    $config = aetherConfig(['poll_interval' => $raw, 'max_poll_attempts' => $raw, 'max_poll_exceptions' => $raw]);
