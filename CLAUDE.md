...
- **SubmitQuantumCircuit::$timeout** is `submit_timeout`, or `process_timeout` + 30 s, and `$failOnTimeout` is true: the local driver simulates inline inside that job, so the attempt must outlive the Python process.
