...
- **Strict Greenfield until v1.0.0**: Never add backward-compatibility shims, legacy config fallbacks, `@deprecated` tags, or artificial nullable arguments with container defaults (`?Service $service = null`, `$service ??= app(...)`) to accommodate hypothetical or intermediate callers. Always use strict typing and direct Dependency Injection, refactoring all call sites directly.
